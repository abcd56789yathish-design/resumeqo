...
import { RotateCcw, ArrowUp, Download, Share2, CheckCircle, AlertTriangle } from "lucide-react";
...
export default function ResultsPage() {
  ...
  const { overallScore=0, scoreBreakdown={}, strongPoints=[], improvements=[], missingKeywords=[], atsScore=0, atsIssues=[], topSuggestion="" } = results;
  const cats = [
  ...
  return (
    <div className="relative z-[1] min-h-screen py-12 px-8">
      <div className="max-w-[680px] mx-auto">
        ...
        <div className="mb-8"><ScoreCard score={animatedScores.overallScore || 0} /></div>
        ...
        {improvements.length > 0 && <div className="mb-6"><ImprovementList improvements={improvements} /></div>}

        {missingKeywords.length > 0 && (
          <div className="bg-[var(--paper-card)] border border-[var(--ink)] shadow-[6px_6px_0_rgba(22,33,61,0.08)] p-8 mb-6">
            <h2 className="font-serif font-[500] text-[18px] text-[var(--ink)] mb-4">🔑 Missing Keywords</h2>
            <div className="flex flex-wrap gap-2">
              {missingKeywords.map((k, i) => (
                <span key={i} className="font-mono text-[12px] bg-[var(--coral-light)] text-[var(--coral-dark)] border border-[var(--coral)] px-3 py-1.5 rounded-[3px]">{k}</span>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="bg-[var(--paper-card)] border border-[var(--ink)] shadow-[6px_6px_0_rgba(22,33,61,0.08)] p-8 mb-6">
          <h2 className="font-serif font-[500] text-[18px] text-[var(--ink)] mb-4">🤖 ATS Score: <span style={{ color: getScoreColor(atsScore) }}>{animatedScores.atsScore || 0}/100</span></h2>
        ...
        {topSuggestion && (
          <div className="bg-[var(--green-light)] border border-[var(--green)] shadow-[6px_6px_0_rgba(22,33,61,0.08)] p-8 mb-6">
            <h2 className="font-serif font-[500] text-[18px] text-[var(--green)] mb-3">💡 Top Tip</h2>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-8">
          ...
          <button onClick={() => window.print()} className="font-mono text-[14px] font-medium bg-[var(--paper-card)] text-[var(--ink)] border border-[var(--line)] py-[15px] px-6 rounded-[3px] hover:bg-[var(--line)] transition-all flex items-center justify-center gap-2">
            <Download className="w-5 h-5" /> Print
          </button>
          <button onClick={() => { navigator.clipboard.writeText(window.location.href); alert("Copied!"); }} className="font-mono text-[14px] font-medium bg-[var(--paper-card)] text-[var(--ink)] border border-[var(--line)] py-[15px] px-6 rounded-[3px] hover:bg-[var(--line)] transition-all flex items-center justify-center gap-2">
