...
export default function Home() {
  return (
    <div className="relative z-[1]">
      {/* ===== HERO ===== */}
      <section className="max-w-[1140px] mx-auto px-6 sm:px-8 grid md:grid-cols-2 gap-10 md:gap-16 items-center pt-16 md:pt-24 pb-16 md:pb-20">
        <div>
          {/* Eyebrow */}
        ...
        <div className="flex justify-center">
          <div className="relative w-full max-w-[340px] bg-[var(--paper-card)] border border-[var(--ink)] shadow-[8px_8px_0_rgba(22,33,61,0.12)] p-7 -rotate-[1.2deg] max-sm:p-5">
            {/* Scan line */}
            ...
            {/* Resume content */}
            <div className="font-serif font-[500] text-[19px] max-sm:text-[16px]">Jordan Reyes</div>
            <div className="text-[11px] text-[var(--ink-soft)] mt-[2px] tracking-[0.02em] max-sm:text-[10px]">
              Product Designer · jordan@email.com
            </div>
            <hr className="border-none border-t border-[var(--line)] my-[14px]" />
            <div className="font-mono text-[9px] tracking-[0.08em] text-[var(--ink-soft)] uppercase mt-[14px] mb-[6px] max-sm:text-[8px]">
              Experience
            </div>
            <div className="h-[7px] bg-[#ECE8DC] rounded-[2px] mb-[6px] w-full max-sm:h-[5px]"></div>
            <div className="h-[7px] bg-[#ECE8DC] rounded-[2px] mb-[6px] w-[90%] max-sm:h-[5px]"></div>
            <div className="h-[7px] bg-[#ECE8DC] rounded-[2px] mb-[6px] w-[80%] max-sm:h-[5px]"></div>
            <div className="font-mono text-[9px] tracking-[0.08em] text-[var(--ink-soft)] uppercase mt-[14px] mb-[6px] max-sm:text-[8px]">
              Skills
            </div>
            <div className="h-[7px] bg-[#ECE8DC] rounded-[2px] mb-[6px] w-[55%] max-sm:h-[5px]"></div>
            <div className="h-[7px] bg-[#ECE8DC] rounded-[2px] mb-[6px] w-[70%] max-sm:h-[5px]"></div>
            <div className="font-mono text-[9px] tracking-[0.08em] text-[var(--ink-soft)] uppercase mt-[14px] mb-[6px] max-sm:text-[8px]">
              Education
            </div>
            <div className="h-[7px] bg-[#ECE8DC] rounded-[2px] mb-[6px] w-[90%] max-sm:h-[5px]"></div>

            {/* Annotations - hidden on small screens, repositioned on medium+ */}
            <div className="hidden md:block absolute top-[118px] -right-[58px] font-mono text-[10.5px] font-semibold px-[7px] py-[3px] rounded-[2px] bg-[var(--coral-light)] text-[var(--coral-dark)] border border-[var(--coral)] opacity-0 translate-y-1 scale-90 animate-[annIn_0.5s_ease_1.1s_forwards]">
              weak verb
            </div>
            <div className="hidden md:block absolute top-[206px] -right-[78px] font-mono text-[10.5px] font-semibold px-[7px] py-[3px] rounded-[2px] bg-[var(--green-light)] text-[var(--green)] border border-[var(--green)] opacity-0 translate-y-1 scale-90 animate-[annIn_0.5s_ease_1.6s_forwards]">
              +keyword match
            </div>
            <div className="hidden md:block absolute top-[280px] -left-[72px] font-mono text-[10.5px] font-semibold px-[7px] py-[3px] rounded-[2px] bg-[var(--coral-light)] text-[var(--coral-dark)] border border-[var(--coral)] opacity-0 translate-y-1 scale-90 animate-[annIn_0.5s_ease_2.1s_forwards]">
              no metrics
            </div>

            {/* Annotation badges for mobile - shown inline */}
            <div className="md:hidden flex flex-wrap gap-1.5 mt-3">
              <span className="font-mono text-[9px] font-semibold px-1.5 py-0.5 rounded-[2px] bg-[var(--coral-light)] text-[var(--coral-dark)] border border-[var(--coral)]">weak verb</span>
              <span className="font-mono text-[9px] font-semibold px-1.5 py-0.5 rounded-[2px] bg-[var(--green-light)] text-[var(--green)] border border-[var(--green)]">+keyword match</span>
              <span className="font-mono text-[9px] font-semibold px-1.5 py-0.5 rounded-[2px] bg-[var(--coral-light)] text-[var(--coral-dark)] border border-[var(--coral)]">no metrics</span>
            </div>

            {/* Score badge */}
            <div className="absolute -bottom-[26px] -left-[26px] w-[84px] h-[84px] rounded-full bg-[var(--ink)] text-[var(--paper)] flex flex-col items-center justify-center border-[3px] border-[var(--paper)] shadow-[4px_4px_0_rgba(22,33,61,0.18)] max-sm:w-[64px] max-sm:h-[64px] max-sm:-bottom-[20px] max-sm:-left-[20px]">
              <div className="font-serif text-[26px] font-semibold leading-[1] max-sm:text-[20px]">87</div>
              <div className="font-mono text-[8px] text-[#A9B0C4] mt-[2px] max-sm:text-[7px]">SCORE</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ===== LOGO STRIP ===== */}
      <div className="border-t border-b border-[var(--line)] py-[22px] px-6 sm:px-8">
      ...
      {/* ===== FEATURES ===== */}
      <section id="features" className="max-w-[1140px] mx-auto px-6 sm:px-8 py-[60px] md:py-[100px]">
        <div className="mb-14">
        ...
        <div className="grid md:grid-cols-3 border border-[var(--line)]">
          {/* Feature 1 */}
          <div className="bg-[var(--paper-card)] p-9 border-r-0 md:border-r border-b md:border-b-0 border-[var(--line)]">
      ...
      {/* ===== HOW IT WORKS (Dark) ===== */}
      <section id="how" className="bg-[var(--ink)] text-[var(--paper)] px-6 sm:px-8 py-[60px] md:py-[100px]">
      ...
      {/* ===== SCORE BREAKDOWN ===== */}
      <section className="px-6 sm:px-8 py-[60px] md:py-[100px]">
      ...
      {/* ===== CTA ===== */}
      <section id="pricing" className="px-6 sm:px-8 pb-[60px] md:pb-[100px]">
        <div className="max-w-[1140px] mx-auto text-center border border-[var(--ink)] bg-[var(--paper-card)] py-[72px] px-6 sm:px-10 relative shadow-[8px_8px_0_var(--coral)]">
      ...
      {/* ===== FOOTER ===== */}
      <footer className="border-t border-[var(--line)] px-6 sm:px-8 py-9">
...
function ScoreRow({ label, pct, pts, color }) {
  return (
    <div className="flex items-center justify-between py-[13px] border-b border-[var(--line)] text-[14px] last:border-b-0 gap-2">
      <div className="flex items-center gap-[10px] min-w-0 shrink-0 max-sm:text-[13px]">
        <span className="w-[8px] h-[8px] rounded-full shrink-0" style={{ background: color }}></span>
        <span className="truncate">{label}</span>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <div className="w-[80px] sm:w-[120px] h-[6px] bg-[#ECE8DC] rounded-[3px] overflow-hidden">
          <div className="h-full rounded-[3px]" style={{ width: `${pct}%`, background: color }}></div>
        </div>
        <span className="font-mono text-[13px] font-semibold w-[34px] text-right shrink-0">{pts}</span>
      </div>
    </div>
  );
}
